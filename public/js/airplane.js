...
var dimondListNum = 0;
var diamondBtnDiv = document.getElementsByClassName('diamondBtnDiv');
...
var game_name_arr = ['定位膽', '大小單雙', '冠亞二星', '冠亞和', '龍虎'];
var game_name_num = 1;
...
var isBeted = false;
var issetAirArr = [];
var contentArr = [null, "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "大", "小", "單", "雙"];
var betListOpen = false;
var betArr = [];
var betIdx = 0;
// let listIdx = 0;
var deleteBet = null;
...
var bsChooseRank = 1;
var oddsArr = [0, 0];
var openBetListBtn = document.getElementById('openBetListBtn');
var openGameBtn = document.getElementById('openGameBtn');
var playBoxisOpen = true;
var bethtmlArr = [];
...
var bsGrandTotalMoneyArr = [0, 0, 0, 0];
var bsGrandTotalMoneyBetedArr = [0, 0, 0, 0];
var betLoading = false;
var betArrString = "";
//lock
var bigLock = false;
var smallLock = false;
var oddLock = false;
var evenLock = false;
var otrank = document.getElementsByClassName('otrank');
var otBetBtn = document.getElementById('otBetBtn');
var all_totalBet = 0;
var all_totalBetNumberCalc = 0;
var all_betArr = [];
var bsChoose = 1;
var betLimitObj = {
  single_term: 0,
...
// let gameStyleNum = 1;
var gameStyleNum = Math.floor(Math.random() * 2) + 99;
...
window.addEventListener('setBetLimit', function (e) {
...
window.addEventListener('updateCycle', function (e) {
  cycleNumber.innerHTML = "\u671F\u865F: ".concat(e.detail.number);
  betListIssue.innerHTML = "\u671F\u865F: ".concat(e.detail.number);
});
...
window.addEventListener('startRun', function (e) {
  ...
  if (new Date().getSeconds() < 50 && new Date().getSeconds() >= 37) {
    ...
    var _loop = function _loop(_i) {
      air[_i].style.animation = "airNo".concat(gameStyleNum, " ").concat(secondsArr[_i][0], "s linear");
...
function timeRun() {
  ...
  if (new Date().getSeconds() == 42) {
    ...
    var _loop2 = function _loop2(i) {
      air[i].style.animation = "airNo".concat(gameStyleNum, " ").concat(secondsArr[i][0], "s linear ");
      if (secondsArr[i][0] > maxSec) {
        maxSec = secondsArr[i][0];
      }
      setTimeout(function () {
...
diamondBoxRight.addEventListener('click', function () {
  ...
  for (var _i8 = 0; _i8 < diamondBtnDiv.length; _i8++) {
    diamondBtnDiv[_i8].style.transform = "translateX(-".concat(dimondListNum * 100, "%)");
  }
});
diamondBoxLeft.addEventListener('click', function () {
  ...
  for (var _i9 = 0; _i9 < diamondBtnDiv.length; _i9++) {
    diamondBtnDiv[_i9].style.transform = "translateX(-".concat(dimondListNum * 100, "%)");
  }
});
...
function initRankFn() {
  for (var _i10 = 0; _i10 < rankingImg.length; _i10++) {
    rankingImg[_i10].src = "/images/airplane/no".concat(_i10 + 1, ".png");
  }
}
...
for (var _i11 = 0; _i11 < rankingImg.length; _i11++) {
  rankingImg[_i11].addEventListener('click', chengRankFn);
}
function notMoneyFn() {
  Swal.fire('警告', '餘額不足', 'error');
}
for (var _i12 = 0; _i12 < rank.length; _i12++) {
  rank[_i12].querySelector('.air').addEventListener('click', guessFn, true);
}
function guessFn(e) {
  ...
  for (var _i13 = 0; _i13 < rank.length; _i13++) {
    rank[_i13].querySelector('.air').removeEventListener('click', guessFn, true);
  }
  ...
  betText.textContent = "+".concat(beyMoney.value);
  ...
  if (game_name_num == 0) {
    //定位膽
    var rankGuessArr = [null, '冠軍', '亞軍', '季軍', '第四名', '第五名', '第六名', '第七名', '第八名', '第九名', '第十名'];
    var content = contentArr[Number(e.target.alt)];
    obj.id = betIdx;
    ...
    //大小單雙
    var _rankGuessArr = [null, '冠軍', '亞軍', '季軍', '第四名', '第五名', '第六名', '第七名', '第八名', '第九名', '第十名'];
    var _content = contentArr[Number(e.target.alt)];
    obj.id = betIdx;
    obj.game = game_name_arr[game_name_num];
    obj.rank = _rankGuessArr[bsChooseRank];
  ...
  for (var _i14 = 0; _i14 < rank.length; _i14++) {
    rank[_i14].querySelector('.air').addEventListener('click', guessFn, true);
  }
  lockBetFn(obj);
}
...
function bsGrandTotalMoneyFn() {
  for (var _i15 = 0; _i15 < bsBtn.length; _i15++) {
    var m = 0;
    m = Number(bsGrandTotalMoneyArr[_i15]) + Number(bsGrandTotalMoneyBetedArr[_i15]);
    bsBtn[_i15].parentNode.querySelector('.grandTotal').innerHTML = "$".concat(m);
  }
}
function airLightFn(arr) {
  var airArr = airplaneRankBox.querySelectorAll('.air');
  arr = arr.map(function (item) {
    return item[0];
  });
  for (var _i16 = 0; _i16 < airArr.length; _i16++) {
    if (arr.includes(Number(airArr[_i16].alt))) {
      // airArr[i].style.filter = "brightness(1.5)"
      airArr[_i16].style.opacity = "1";
    } else {
      // airArr[i].style.filter = "brightness(.6)"
      airArr[_i16].style.opacity = ".5";
    }
  }
  for (var _i17 = 0; _i17 < bsBtn.length; _i17++) {
    if (arr.includes(Number(bsBtn[_i17].alt))) {
      // bsBtn[i].style.filter = "brightness(1.5)"
      bsBtn[_i17].style.opacity = "1";
    } else {
      bsBtn[_i17].style.opacity = ".5";
    }
  }
}
function removeBetFn(e) {
  var node = e.target.parentNode.parentNode;
  var itemMoney = Number(e.target.parentNode.querySelectorAll('p')[3].innerText);
  arr = issetAirArr.map(function (item) {
    return item[0];
  });
  var idx = 0;
  var bsText = e.target.parentNode.querySelectorAll('p')[1].innerText.split('-')[1].trim();
  if (bsText == "大") {
    idx = indexOf(arr, 11);
  ...
  var betIdx = e.target.parentNode.querySelector('.betIdx').value;
  betArr = betArr.filter(function (item) {
    if (item.id == betIdx) {
...
function removeLockFn() {
  var issetBig = false;
  var issetSmall = false;
  var issetOdd = false;
  var issetEven = false;
  for (var _i18 = 0; _i18 < betArr.length; _i18++) {
    if (betArr[_i18].content == "大") {
      issetBig = true;
    } else if (betArr[_i18].content == "小") {
      issetSmall = true;
    } else if (betArr[_i18].content == "單") {
      issetOdd = true;
    } else if (betArr[_i18].content == "雙") {
      issetEven = true;
    }
  }
  for (var _i19 = 0; _i19 < all_betArr.length; _i19++) {
    if (all_betArr[_i19].content == "大") {
      issetBig = true;
    } else if (all_betArr[_i19].content == "小") {
      issetSmall = true;
    } else if (all_betArr[_i19].content == "單") {
      issetOdd = true;
    } else if (all_betArr[_i19].content == "雙") {
      issetEven = true;
    }
  }
  if (!issetBig) {
...
function randerBetHtml(obj) {
  ...
  var isBet = document.createElement("input");
  var span1 = document.createElement("span");
  var text1 = document.createTextNode("下注項目");
  span1.appendChild(text1);
  var p1 = document.createElement("p");
  var content1 = document.createTextNode(obj.game);
  p1.appendChild(content1);
  var span2 = document.createElement("span");
  var text2 = document.createTextNode("下注內容");
  span2.appendChild(text2);
  var p2 = document.createElement("p");
  var content2 = document.createTextNode("".concat(obj.rank, " - ").concat(obj.content));
  p2.appendChild(content2);
  var span3 = document.createElement("span");
  var text3 = document.createTextNode("賠率");
  span3.appendChild(text3);
  var p3 = document.createElement("p");
  var content3 = document.createTextNode(obj.odds);
  p3.appendChild(content3);
  var span4 = document.createElement("span");
  var text4 = document.createTextNode("投注金額");
  span4.appendChild(text4);
  var p4 = document.createElement("p");
  var content4 = document.createTextNode(obj.money);
...
function chkBtnFn() {
  if (betLoading) {
    Swal.fire('下注失敗', '無法取得用戶資訊，請重試一遍', 'error');
    return;
  }
  if (isLock) {
    Swal.fire('下注失敗', '您的錢包已被鎖定', 'error');
    return;
  }
  if (!isBetTime) {
    Swal.fire('下注失敗！', '現在非下注時間', 'error');
    return;
  }
  if (totalBet <= 0) {
  ...
  var item = listAll.querySelectorAll('.item');
  all_totalBet += totalBet;
  all_totalBetNumberCalc += totalBetNumberCalc;
  all_betArr = [].concat(_toConsumableArray(all_betArr), _toConsumableArray(betArr));
  var odds = oddsArr[0];
  var bsOdds = oddsArr[1];
  ...
  for (var _i20 = 0; _i20 < item.length; _i20++) {
    if (item[_i20].querySelector('.deleteBet')) {
      item[_i20].removeChild(item[_i20].querySelector('.deleteBet'));
    }
    item[_i20].querySelector(".isBet").value = 1;
  }
  ...
  totalBetNumberCalc = 0;
  betArr = [];
  for (var _i21 = 0; _i21 < bsGrandTotalMoneyBetedArr.length; _i21++) {
    bsGrandTotalMoneyBetedArr[_i21] += bsGrandTotalMoneyArr[_i21];
  }
  bsGrandTotalMoneyArr = [0, 0, 0, 0];
  issetAirArr.forEach(function (item) {
    item[1] = true;
  });
}
window.addEventListener('betSuccess', function (e) {
  if (e.detail.bool) {
    Swal.fire('投注成功！', '等待整點開獎', 'success');
  } else {
    Swal.fire('下注失敗！', "\u7121\u6CD5\u53D6\u5F97\u7528\u6236\u8CC7\u8A0A\uFF01\u8ACB\u518D\u8A66\u4E00\u904D", 'error');
  }
  betLoading = false;
});
function calcBetFn() {
...
window.addEventListener('updateMyMoneyHtml', function (e) {
  ...
  winMessage.innerHTML = "+ ".concat(Math.round(e.detail.win));
});
reBtn.addEventListener('click', function () {
  ...
  betArr = [];
  var item = listAll.querySelectorAll('.item');
  issetAirArr = issetAirArr.filter(function (item) {
    return item[1] === true;
  });
  airLightFn(issetAirArr);
  bsGrandTotalMoneyArr = [0, 0, 0, 0];
  bsGrandTotalMoneyFn();
  for (var _i22 = 0; _i22 < item.length; _i22++) {
    if (item[_i22].querySelector('.isBet').value == 0) {
      item[_i22].parentNode.removeChild(item[_i22]);
    }
  }
});
doubleBtn.addEventListener('click', function () {
  ...
  var newMoney = Number(myDoller.innerHTML) - all_totalBet;
  if (newMoney < 0) {
    Swal.fire('警告', '餘額不足', 'error');
    return;
  }
  ...
  totalBetNumberCalc = 0;
  betArr = [];
  for (var _i23 = 0; _i23 < bsGrandTotalMoneyBetedArr.length; _i23++) {
    bsGrandTotalMoneyBetedArr[_i23] = bsGrandTotalMoneyBetedArr[_i23] * 2;
  }
  bsGrandTotalMoneyFn();
  Swal.fire('投注成功', '加倍下注成功，等待整點開獎', 'success');
  return;
});
function airTopTenHTML(nowAnswer) {
  for (var _i24 = 0; _i24 < airNum.length; _i24++) {
    airNum[_i24].src = "/images/airplane/airRank".concat(nowAnswer[_i24], ".png");
  }
}
var threeArr = [1, 0, 2];
function airTopThreeHTML(nowAnswer) {
  for (var _i25 = 0; _i25 < topThreeAir.length; _i25++) {
    topThreeAir[_i25].src = "/images/airplane/airRank".concat(nowAnswer[threeArr[_i25]], ".png");
  }
}
...
for (var _i26 = 0; _i26 < bsBtn.length; _i26++) {
  bsBtn[_i26].addEventListener('mousedown', downBsBtnFn);
}
for (var _i27 = 0; _i27 < bsBtn.length; _i27++) {
  bsBtn[_i27].addEventListener('mouseup', upBsBtnFn);
}
for (var _i28 = 0; _i28 < bsBtn.length; _i28++) {
  bsBtn[_i28].addEventListener('click', guessFn);
}
...
for (var _i29 = 0; _i29 < rankingImgBs.length; _i29++) {
  rankingImgBs[_i29].addEventListener('click', chengBsRankFn);
}
...
function initBsRankFn() {
  for (var _i30 = 0; _i30 < rankingImgBs.length; _i30++) {
    rankingImgBs[_i30].src = "/images/airplane/no".concat(_i30 + 1, ".png");
  }
}
...
for (var _i31 = 0; _i31 < otrank.length; _i31++) {
  otrank[_i31].addEventListener('click', betotFn);
}
var otGuessArr = {
  "no1": 0,
  "no2": 0
...
function betotFn(e) {
  var ot = e.target.parentNode.parentNode.querySelectorAll('.otrank');
  for (var _i32 = 0; _i32 < ot.length; _i32++) {
    ot[_i32].querySelector('.medal').style.display = "none";
  }
...
window.addEventListener('isbetedFn', function (e) {
  var html = '';
  e.detail.blArr.forEach(function (item) {
    totalBet += item.money;
    totalBetNumberCalc += item.chips;
    item.bet_info.forEach(function (el) {
      betIdx++;
      html += "<div class=\"item\">\n            <i class=\"fas fa-times deleteBet\"></i>\n            <input type=\"hidden\" value=\"".concat(betIdx, "\" class='betIdx'>\n            <span>\u4E0B\u6CE8\u9805\u76EE:</span><br>\n            <p>").concat(item.bet_info[0], "</p>\n            <span>\u4E0B\u6CE8\u5167\u5BB9:</span>\n            <p></p>\n            <span>\u8CE0\u7387:</span>\n            <p></p>\n            <span>\u6295\u6CE8\u91D1\u984D:</span>\n            <p></p>\n            </div>");
    });
  });
...
window.addEventListener('pointLockFn', function (e) {
  isLock = true;
});
...
window.addEventListener('initBetArrFn', function (e) {
  var initBetArr = e.detail.betArr.map(function (item) {
    return JSON.parse(item).map(function (arr) {
      return arr;
    });
  });
...
function initLockFn() {
  all_betArr.forEach(function (item) {
  ...
  var m = 0;
  for (var _i33 = 0; _i33 < bsBtn.length; _i33++) {
    m = 0;
    m = Number(bsGrandTotalMoneyBetedArr[_i33]);
    bsBtn[_i33].parentNode.querySelector('.grandTotal').innerHTML = "$".concat(m);
  }
  airLightFn(issetAirArr);
  totalBetNmuber.innerHTML = all_totalBetNumberCalc;
  totalBetMoney.innerHTML = all_totalBet;
  for (var _i34 = 0; _i34 < all_betArr.length; _i34++) {
    if (all_betArr[_i34].content == '大') {
      smallLock = true;
      smallBtn.classList.add('lock');
    } else if (all_betArr[_i34].content == "小") {
      bigLock = true;
      bigBtn.classList.add('lock');
    } else if (all_betArr[_i34].content == "單") {
      evenLock = true;
      evenBtn.classList.add('lock');
    } else if (all_betArr[_i34].content == "雙") {
      oddLock = true;
      oddBtn.classList.add('lock');
    }
  }
...
setTimeout(function () {
  window.Echo.channel('game-status').listen('.GameStatus', function (e) {
    watchStatus(e.status);
  });
  window.Echo.channel('user-status').listen('.UserStatus', function (e) {
    if (e.user_id === authId) {
      watchStatus(e.status);
    }
  });
  window.Echo.channel('users-status').listen('.UsersStatus', function (e) {
    watchStatus(e.status);
  });
  window.Echo.channel('set-bet-limit').listen('.SetBetLimit', function (e) {
    setBetLimit(e);
  });
}, 200);
})();
